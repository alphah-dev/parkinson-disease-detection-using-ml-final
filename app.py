...
feature_mapping = {
    ...
    'jitter_percent': 'MDVP:Jitter(%)', 'jitter_abs': 'MDVP:Jitter(Abs)', 'rap': 'MDVP:RAP',
    'ppq': 'MDVP:PPQ', 'jitter_ddp': 'Jitter:DDP', 'shimmer': 'MDVP:Shimmer',
    'shimmer_db': 'MDVP:Shimmer(dB)', 'apq3': 'Shimmer:APQ3', 'apq5': 'Shimmer:APQ5',
    'mdvp_apq': 'MDVP:APQ', 'shimmer_dda': 'Shimmer:DDA', 'nhr': 'NHR', 'hnr': 'HNR',
    'rpde': 'RPDE', 'dfa': 'DFA', 'spread1': 'spread1', 'spread2': 'spread2', 'd2': 'D2',
    'ppe': 'PPE'
}
    ...
    if 'file' in request.files and request.files['file'].filename != '':
        ...
        if file and file.filename.endswith('.xlsx'):
            ...
            try:
                ...
                first_row = df[expected_features].iloc[0].to_dict()
                ...
                auto_fill_data = {key: first_row[value] for key, value in feature_mapping.items()}

                os.remove(file_path)
                return jsonify({'predictions': results, 'auto_fill': auto_fill_data})
